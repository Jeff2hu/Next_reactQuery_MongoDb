...
export function CardWithForm({data}: Props) {
  ...
  const deleteHandler = useCallback(() => {
    setAlert({open:true, text:"Would you like to delete this topic?", title:"Waring", ok: () => deleteMutation.mutate()})
  }, [])

  return (
    <Card className="w-full">
      <CardHeader className={`${collapsible ? "" : "pb-0 pt-8"} flex flex-row justify-between items-end transition-all`}>
